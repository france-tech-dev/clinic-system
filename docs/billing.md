...
| Features                      | **Todas incluídas** em todos os planos — diferenciação só por seats                |
| Preços                        | Catálogo público em `BILLING_PLAN_PRICES_BRL`; Stripe via `STRIPE_PRICE_*` no host |

## Planos

| Plano (UI)       | Enum         | Profissionais incluídos | Mensalidade | Extra                                   |
| ---------------- | ------------ | ----------------------- | ----------- | --------------------------------------- |
| **Solo**         | `SOLO`       | 1                       | R$ 99       | — (hard cap; upgrade para Professional) |
| **Professional** | `PRO`        | até 3                   | R$ 229      | — (hard cap; upgrade para Enterprise)   |
| **Enterprise**   | `ENTERPRISE` | até 9                   | R$ 599      | +R$ 59/mês por profissional adicional   |

Trial ignora plano e limite de profissionais. `OrganizationBilling.extraSeats` sincroniza com a quantity do Price `STRIPE_PRICE_EXTRA_SEAT` na assinatura.

## Fluxo

1. Criar clínica → Customer + Subscription com trial 7 dias (preço Solo só como item Stripe; entitlements = tudo).
...
- Catálogo (preços + seats): `src/shared/constants/billing-plans.ts`
- Persistência / Stripe: `src/domains/billing/`
...
STRIPE_PRICE_EXTRA_SEAT="price_..."  # profissional adicional R$ 59
```
