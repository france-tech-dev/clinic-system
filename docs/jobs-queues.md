...
**Estado actual:** processamento síncrono (ex. logo via `shared/lib/media`). Sem broker de filas.
...
3. Introduzir BullMQ:
   - `REDIS_URL`
   - processo worker separado (ou mesmo deploy com `worker` entry) no Dokploy
...
- [ ] `pnpm add bullmq ioredis` (ou equivalente actual)
- [ ] Módulo `shared/lib/jobs` (ou `server/jobs`): queues, enqueue, tipos de payload
- [ ] Processo worker no deploy
